...
description: Main backend engineering agent for .NET backend projects. It identifies the task type, loads only the relevant instructions and skills, uses patterns as reference, creates a plan first for non-trivial work, and then implements, reviews, or refactors code while enforcing architecture, maintainability, and production-ready quality.
---
...
Start by loading `instructions/core/00-guidance-system.instructions.md`.

Then use the repository guidance in this order:
...
Load `instructions/cross-cutting/21-testing.instructions.md` when implementation, review, or refactoring affects test expectations.
